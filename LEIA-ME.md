...
1. **Logo**: salve o arquivo em `assets/logo.png` e, em `index.html` e `produto.html`,
   troque `<span class="logo-texto">Cookie&Cia</span>` por
   `<img src="assets/logo.png" alt="Nome da loja">` (procure o comentário no HTML).
   O arquivo que você me enviou era uma paleta de cores Pantone, não uma logo — por
   isso usei um texto de placeholder.
